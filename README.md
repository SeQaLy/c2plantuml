...
1. **生成画像のプレビュー**: GUI に画像表示ペインを設け、変換後すぐ確認（クリックで拡大）。
2. **関数フィルタ**: 正規表現で対象関数を絞り込み（例: `^drv_` だけ出力、テスト関数を除外）。
3. **条件ラベルの自動化**: `then (yes)/else (no)` を、条件式から推定（`== NULL` → `null/not null` 等）。
4. **コメントを図に反映**: 直前の行コメントを `note` として該当アクティビティに付与。
5. **関数呼び出しの強調**: 関数呼び出しだけ色付け、もしくは呼び出し先一覧（コールグラフ）出力。
6. **ファイル単位の 1 図モード**: 1 ファイルの全関数を縦に並べた俯瞰図（`partition` で関数ごとに区切る）。
7. **ドラッグ＆ドロップ入力**（`tkinterdnd2` 使用、要追加インストール）。
8. **SVG/PNG 一括 ZIP 出力**、**Mermaid 形式への同時出力**。
9. **行番号の併記**: 各アクティビティに元ソースの行番号を添える（トレーサビリティ）。
